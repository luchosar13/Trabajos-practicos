...
class Grafo():
    ...
    def episodios_comp(self):
        jedis = {}
        visitados = []
        aux = self.__inicio
        while aux is not None:
            visitados.append(aux.info)
            auxAdyacentes = aux.adyacentes.get_inicio()
            while auxAdyacentes is not None:
                if (auxAdyacentes.info not in visitados):
                    jedis[f'{aux.info}-{auxAdyacentes.info}'] = auxAdyacentes.peso
                auxAdyacentes = auxAdyacentes.sig
            aux = aux.sig

        while jedis is not None:
            if (visitados['peso'] >= 2):
                print(jedis)
    ...
    def contar_maravillas(self):
        ...
        while aux is not None:
            if aux.datos['pais'] not in paises:
                print('-')
                paises[aux.datos['pais']] = {'a': False, 'n': False}

            if aux.datos['tipo'] == 'n':
                paises[aux.datos['pais']]['n'] = True
            else:
                paises[aux.datos['pais']]['a'] = True
            aux = aux.sig
